...
static Pos
homePlace(int i, int j) {
  return {i * 2 + 3, j + 2};
}

static bool
isHome(const Places &p, int i, int j) {
  return p[i][j].y > 1 && p[i][j].x == (i * 2 + 3);
}
...
solve(const std::vector<string> lines) {
  ...
  while (!q.empty()) {
    ...
    for (int i = 0; i < p.size(); ++i) {
      ...
      for (int j = 0; j < hp; ++j) {
        if (isHallway(p[i][j])) {
          if (!tryMove(q, dists, dist, p, i, j, homePlace(i, hp - 1))) {
            for (int k = hp - 1; k > 0; --k) {
              if (at(p, homePlace(i, k)) != i || tryMove(q, dists, dist, p, i, j, homePlace(i, k - 1))) {
                break;
              }
            }
          }
