...
struct State : StateBase {
  ...
  auto robots() const { return first; }
  uint robots(Type r) const { return first[r]; }
  auto counts() const { return second; }
  uint counts(Type t) const { return second[t]; }
};

template <>
struct std::hash<State> {
  size_t operator()(const State &state) const noexcept { return *reinterpret_cast<const size_t *>(&state); }
};
