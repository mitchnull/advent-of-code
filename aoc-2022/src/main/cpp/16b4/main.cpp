...
using Pressures = std::unordered_map<size_t, uint>;
...
main() {
  ...
  uint res = 0;
  for (auto [mePath, mePressure]: pressures) {
    for (auto [nelliePath, nelliePressure]: pressures) {
      if (!(mePath & nelliePath)) {
        res = std::max(res, mePressure + nelliePressure);
      }
    }
  }
  std::cout << res << "\n";
