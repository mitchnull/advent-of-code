...
struct MapCache {
  ...
  const Map &advance(const Map &map) {
    ++advanceCount;
#if 1
    if (map != **last) {
  ...
  void debug() { std::cout << "advance called " << advanceCount << " times, size: " << cache.size() << "\n"; }
private:
...
main() {
  ...
  visited[start.mapPos.pos.y][start.mapPos.pos.x].push_back(map);
  while (!queue.empty()) {
