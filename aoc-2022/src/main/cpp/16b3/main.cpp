...
main() {
  ...
  Visited visited;
  std::vector<uint> pressures;
  uint visitedEnd = 1 << (nodes.size() - 1);
  for (uint64 v = 0; v < visitedEnd; ++v) {
    visited = v << 1;
    pressures.push_back(maxPressure(nodes, 0, TURNS + 1, visited));
  }

  uint res = 0;
  uint mask = visitedEnd - 1;
  for (uint64 split = 0; split < visitedEnd; ++split) {
    uint me = pressures[split];
    uint nellie = pressures[~split & mask];
