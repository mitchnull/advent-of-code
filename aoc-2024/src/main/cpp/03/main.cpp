...
main() {
  ...
  int res1 = 0;
  for (auto it = std::sregex_token_iterator(text.begin(), text.end(), r, {1, 2}), end = std::sregex_token_iterator();
      it != end;
      ++it) {
  ...
  int res2 = 0;
  for (auto it = std::sregex_token_iterator(text2.begin(), text2.end(), r, {1, 2}), end = std::sregex_token_iterator();
      it != end;
      ++it) {
